...
counts = 1
for filename in os.listdir('../data/test_label/'):
    filename = '../data/test_label/' + filename
    ori_img1 = cv2.imread(filename)
    # ori_img1 = cv2.cvtColor(ori_img1, cv2.COLOR_BGR2GRAY)

    if np.max(ori_img1) == 0:
        print('{} is none'.format(filename))
        print(counts)
        counts += 1
        continue
    ...
    for i in range(1, num_labels):
        ...
        cv2.polylines(img=ori_img, pts=[regularization_contour], isClosed=True, color=(255, 0, 0), thickness=5)

        single_out = np.zeros_like(ori_img)
    ...
    cv2.imwrite('./testlabel_out/' + filename[-14:], ori_img1)
