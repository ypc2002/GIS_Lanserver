...
def pldist(x0, x1, x2):
    ...
    """
    x01, x11, x21 = x0[:2], x1[:2], x2[:2] # discard timestamp
    if x11[0] == x21[0]:
        return np.abs(x01[0] - x11[0])

    return np.divide(np.linalg.norm(np.linalg.det([x21 - x11, x11 - x01])),
                     np.linalg.norm(x21 - x11))
